...
export function SourceChart({
  ...
  const aggregatedIncomeSources = useMemo(() => {
    if (!data) return [];
    return aggregateBySource({ transactions: data, type: "income" });
  }, [data]);
  const aggregatedExpenseSources = useMemo(() => {
    if (!data) return [];
    return aggregateBySource({ transactions: data, type: "expense" });
  }, [data]);
  ...
  return (
    <Card className="flex flex-col">
      ...
      <CardContent className="flex flex-row flex-wrap items-center justify-center  pb-0">
        {aggregatedIncomeSources.length > 0 && (
          <ChartContainer
            config={{}}
                    ...
                    if (viewBox && "cx" in viewBox && "cy" in viewBox) {
                      return (
                        ...
                        >
                          ...
                          >
                            {aggregatedIncomeSources.reduce(
                              (acc, { value }) => {
                                acc += value;
                                return acc;
                              },
        ...
        {aggregatedExpenseSources.length > 0 && (
          <ChartContainer
            config={{}}
            className="mx-auto aspect-square w-full max-w-[300px]"
          >
                      ...
                      return (
                        ...
                        >
                          <tspan
                            ...
                            className="fill-foreground text-3xl font-bold"
                          >
                            {aggregatedExpenseSources.reduce(
                              (acc, { value }) => {
                                acc += value;
                                return acc;
                              },
...
function aggregateBySource({
  ...
  const sourceMap = transactions
    .filter((item) => item.type === type)
    .reduce<Record<string, AggregatedSource>>(
      (acc, { amount, source, type }) => {
        if (!acc[source.id]) {
          acc[source.id] = {
            name: source.name,
            id: source.id,
            type: type,
            value: 0,
            fill: generateColor(source.id),
          };
        }

        acc[source.id].value += amount;
        return acc;
      },
      {}
    );
